...
import { CheckLg, Fire, ArrowRight } from "react-bootstrap-icons";
  ...
  return (
    <div className="orbit-card">
        ...
        {items.map((item, index) => {
          if (item.key === "module" && item.widgetModule) {
            return (
              <div
                key={item.key}
                className={`orbit-checklist__item orbit-checklist__item--module ${item.done ? "orbit-checklist__item--done" : ""}`}
                onClick={item.action}
              >
                ...
                <div className="orbit-checklist__module-widget">
                  <div className="orbit-checklist__module-widget-top">
                    <p className={`orbit-checklist__label ${item.done ? "orbit-checklist__label--done" : ""}`}>
                      {item.isHotModule && <Fire size={12} color="var(--amber-glow)" className="orbit-checklist__hot-icon" />}
                      {item.widgetModule.title}
                    </p>
                    <span className="orbit-checklist__resume-btn">
          ...
          return (
            <div
              key={item.key}
              className={`orbit-checklist__item ${item.done ? "orbit-checklist__item--done" : ""}`}
              onClick={item.action}
            >
              <span className={`orbit-checklist__bullet ${item.done ? "orbit-checklist__bullet--done" : ""}`}>
                {item.done ? <CheckLg size={12} /> : index + 1}
      ...
      <p className="orbit-checklist__footnote">
        {anyDone
          ? "Nice — today already counts toward your consistency streak! \u{1F389}"
